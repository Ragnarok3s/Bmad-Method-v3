...
## Problema
A suíte `pytest` falha na coleta porque `backend/services/core/services/__init__.py` importa `record_dashboard_alerts`, mas a função não está definida em `backend/services/core/metrics.py`.

## Evidências
- Execução de `pytest` resulta em `ImportError: cannot import name 'record_dashboard_alerts'`.
- O arquivo `backend/services/core/metrics.py` não declara a função esperada.
...
## Ações recomendadas
1. Implementar a função `record_dashboard_alerts` (ou ajustar o import) em `backend/services/core/metrics.py`.
