...
| Ameaça | Descrição | Mitigação | Owner |
| ------ | --------- | --------- | ----- |
| Spoofing cruzado entre tenants | Tentativa de login com credenciais válidas fora do tenant associado | Repositório `TenantAccessRepository` valida vínculo na tabela `tenant_agent_access` antes de delegar ao `AuthenticationService`; auditado pela suíte `pytest tests/backend/services/identity -q`. | Security Champion |
| Repudiation | Agente alega não ter sofrido bloqueio ou MFA adicional | Eventos `auth_login_blocked` e `auth_mfa_failed` enviados para observabilidade conforme descrito em [`backend/services/identity/api.py`](../../backend/services/identity/api.py) e rastreados no [roadmap](../product-roadmap.md#atualização-2025-02-15-identidade-multi-tenant). | Observability Lead |
