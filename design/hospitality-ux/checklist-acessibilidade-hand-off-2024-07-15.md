...
## Contexto
- **Protótipos**: fluxos web (dashboard, reservas, calendário, faturação, housekeeping, gestor) entregues no Figma.
...
| Feedback auditivo/visual em estados críticos | Vídeo `incidents-audible-feedback.mp4` (pasta Evidências — SharePoint) e walkthrough em navegador com VoiceOver. | ✅ | Modal acessível com foco automático validado em leitores de ecrã. |
| Compatibilidade com leitor de ecrã | Vídeo `housekeeping-voiceover.mp4` (pasta Evidências — SharePoint) validando a rota web. | ✅ | Ordem de leitura do card ajustada (título → estado → reserva → notas → ação). |
| Switch Control (varredura assistiva) | Vídeo `housekeeping-switch-control.mp4` (pasta Evidências — SharePoint) usando controles de navegação por teclado. | ✅ | Botão de alternar estado da tarefa acionável com foco programático. |
...
2. **Descrição alternativa do ícone de sincronização offline (Web)** — Owner: QA Acessibilidade (prazo 2024-07-17). Status: Concluída (aguardando validação final no dev).
3. **Ordem de leitura do card de quartos (Web)** — Owner: Engenharia Frontend (prazo 2024-07-22). Status: Concluída; evidência documentada em vídeo `housekeeping-voiceover.mp4`.
