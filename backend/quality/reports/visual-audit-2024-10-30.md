...
- Todas as métricas chave exibem "Ocorreu um erro inesperado ao carregar as métricas do dashboard." sempre que a Core API não responde, deixando o painel inutilizável.【F:frontend/app/page.tsx†L138-L199】【F:frontend/services/api/dashboard.ts†L1-L69】
- O mesmo padrão ocorre para SLAs dos parceiros, com mensagens genéricas e nenhum dado de fallback.【F:frontend/app/page.tsx†L365-L408】【F:frontend/services/api/partners.ts†L1-L80】
...
- Ficheiros `.woff2` gerados têm 16 bytes, provocando warnings "Failed to decode downloaded font" e regressão visual para fontes de sistema.【F:frontend/app/layout.tsx†L1-L26】【bd6a05†L1-L4】
...
- O `TelemetryProvider` inicialmente inicializava exportadores OTLP apontando para `http://localhost:4318/v1/*` sem verificar disponibilidade.【F:frontend/telemetry/init.ts†L1-L119】
- Atualização: a inicialização passou a validar `NEXT_PUBLIC_ENABLE_TELEMETRY` e `NEXT_PUBLIC_OTEL_EXPORTER_OTLP_ENDPOINT`, registrando apenas instrumentação local quando o collector não está configurado e evitando requisições falhadas.【F:frontend/telemetry/init.ts†L60-L119】【F:frontend/components/telemetry/TelemetryProvider.tsx†L12-L21】
...
- Lint aponta dependências instáveis em `usePageTour` e `TourContext`, o que pode causar re-renderizações infinitas ou estado inconsistente ao navegar.【098d19†L1-L17】【F:frontend/components/tour/usePageTour.ts†L8-L36】【F:frontend/components/tour/TourContext.tsx†L240-L296】
...
- `TenantProvider` recria `resolvedCapabilities` fora de um `useMemo`, invalidando memoizações descendentes e podendo causar flicker de banners dependentes dessas flags.【098d19†L8-L16】【F:frontend/lib/tenant-context.tsx†L63-L105】
